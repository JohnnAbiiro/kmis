...
class SubjectRegistration extends StatefulWidget {
  ...
  /// When true, this widget renders without its own Scaffold/AppBar so it
  /// can be dropped straight into a `Dialog` (e.g. from the setup wizard).
  /// On success it calls `Navigator.pop(context, savedSubject)` instead of
  /// navigating to the courses list, so the caller gets the result back.
  final bool embedded;

  const SubjectRegistration({super.key, this.subject, this.embedded = false});
...
class _SubjectRegistrationState extends State<SubjectRegistration> {
  ...
  @override
  Widget build(BuildContext context) {
    final content = _isLoadingOptions
        ? const Center(child: Padding(padding: EdgeInsets.all(40), child: CircularProgressIndicator()))
        : Consumer<Myprovider>(builder: (context, provider, _) => _form(provider));

    if (widget.embedded) return content;

    return Scaffold(
      appBar: AppBar(
        title: Text(widget.subject != null ? 'Edit course / subject' : 'Register course / subject'),
        leading: IconButton(
          icon: const Icon(Icons.arrow_back),
          onPressed: () {
            if (context.canPop()) {
  ...
  Widget _form(Myprovider provider) {
    final isEdit = widget.subject != null;
    final colorScheme = Theme.of(context).colorScheme;
    final departmentOptions = provider.departments
        .map((item) => item.name)
                  ...
                  Padding(
                    ...
                    child: Row(
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            isEdit ? 'Edit course / subject' : 'Register course / subject',
