...
class _SubjectRegistrationState extends State<SubjectRegistration> {
  ...
  Future<void> _loadFaculties(Myprovider provider) async {
    ...
    for (final doc in results[1].docs) {
      ...
      departmentFaculties[entry['name'].toString()] =
          entry['faculty']?.toString() ?? '';
    }
    setState(() {
      _facultyEntries = results[0].docs
          .map((doc) => (doc.data())['name'].toString())
          .toSet()
          .toList();
      _departmentFaculties = departmentFaculties;
      // Prefill faculty from the department when editing an existing record.
      ...
      child: Consumer<Myprovider>(
        builder: (context, provider, _) {
          final departmentOptions = provider.departments
              ...
              .where(
                (name) =>
            _faculty == null || _departmentFaculties[name] == _faculty,
          )
              .toSet()
              .toList();
                      ...
                      children: [
                        ...
                        _dropdown(
                          ...
                          _facultyEntries,
                            ...
                            _faculty = value;
                            if (_department != null &&
                                _departmentFaculties[_department] != _faculty) {
                              _department = null;
                            }
                          }),
                          required: true,
  ...
  Future<void> _save(Myprovider provider) async {
    ...
    // Use the same schoolId_code scheme as the rest of the app so edits
    // always target the correct existing document instead of drifting.
    final id = '${provider.schoolid}_$code';
    final progress = ProgressHUD.of(context);
    progress?.show();

    // If editing and the code changed (so the doc id changed), remove the old record.
    ...
    // Force these in explicitly so they always persist regardless of toMap().
    data['code'] = code;
