...
class _SetupWizardPageState extends State<SetupWizardPage> {
  ...
  Future<void> _registerAcademicYear() async {
    ...
    final name = await showDialog<String>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Register academic year'),
        content: TextField(
    ...
    final provider = context.read<Myprovider>();
    final compactName = name.replaceAll('/', '').replaceAll(RegExp(r'\s+'), '');
    final id = '${provider.schoolid}_${compactName.toLowerCase()}';
    final year = AcademicModel(
      id: id,
      idd: compactName,
      name: name,
      staff: provider.name,
    ...
    await provider.db
        ...
        .doc(id)
        .set(year.toMap(), SetOptions(merge: true));
    await provider.fetchacademicyear();
    if (mounted) {
      _yearController.text = name;
      setState(() {});
    }
  }
      ...
      case 0:
        ...
        return _fields([
          ...
          _registeredSettingChoice(
            ...
            onChanged: (value) =>
                setState(() => _yearController.text = value ?? ''),
