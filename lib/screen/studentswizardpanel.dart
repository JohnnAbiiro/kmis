...
//       final matchesSearch = query.isEmpty || s.name.toLowerCase().contains(query) || s.studentid.toLowerCase().contains(query);
//       final matchesDepartment = _departmentFilter == null || s.department == _departmentFilter;
//       final matchesClass = _classFilter == null || s.level == _classFilter;
...
//     String? department = initial != null && initial.department.isNotEmpty ? initial.department : null;
//     String? level = initial != null && initial.level.isNotEmpty ? initial.level : null;
...
//                   const SizedBox(height: 12),
//                   DropdownButtonFormField<String>(
//                     initialValue: provider.departments.any((d) => d.name == department) ? department : null,
//                     decoration: const InputDecoration(labelText: 'Department'),
//                     items: provider.departments.map((d) => DropdownMenuItem(value: d.name, child: Text(d.name))).toList(),
//                     onChanged: (v) => setModalState(() => department = v),
//                   ),
//                   const SizedBox(height: 12),
//                   DropdownButtonFormField<String>(
//                     initialValue: provider.classdata.any((c) => c.name == level) ? level : null,
//                     decoration: const InputDecoration(labelText: 'Class / Level'),
//                     items: provider.classdata.map((c) => DropdownMenuItem(value: c.name, child: Text(c.name))).toList(),
...
//             FilledButton.icon(
//               onPressed: nameController.text.trim().isEmpty || idController.text.trim().isEmpty || department == null || level == null || sex == null
//                   ? null
//                   : () async {
...
//                   items: [const DropdownMenuItem(value: null, child: Text('All departments')), ...provider.departments.map((d) => DropdownMenuItem(value: d.name, child: Text(d.name)))],
...
//                   items: [const DropdownMenuItem(value: null, child: Text('All classes')), ...provider.classdata.map((c) => DropdownMenuItem(value: c.name, child: Text(c.name)))],
          ...
          actions: [
            ...
            FilledButton.icon(
              onPressed: nameController.text.trim().isEmpty || idController.text.trim().isEmpty || department == null || level == null || sex == null
                ...
                final selectedDepartment = provider.departments.where((d) => d.name == department).firstOrNull;
                final selectedClass = provider.classdata.where((c) => c.name == level).firstOrNull;
                ...
                final model = StudentModel(
                  ...
                  departmentid: selectedDepartment?.id ?? initial?.departmentid ?? '',
                  departmentname: department ?? '',
                  classid: selectedClass?.id ?? initial?.classid ?? '',
                  classname: level ?? '',
                );
                await provider.db.collection('students').doc(id).set(model.toMap(), SetOptions(merge: true));
                provider.upsertStudent(model);
